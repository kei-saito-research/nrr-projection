...
\textit{Local draft. Part of the Non-Resolution Reasoning (NRR) research program.}}
...
\begin{proof}
$Y_1$ is a deterministic function of $Y_k$ obtained by selecting one coordinate. Mutual information is monotone under deterministic post-processing, so
\begin{equation}
I(S;Y_k) \geq I(S;Y_1).
\end{equation}
\end{proof}
...
Let $C_j : \mathcal{S}_E \to \mathcal{S}_E^{(j)}$ be a destructive collapse operator. For any state functional $F$ that depends on alternative-sensitive structure, there exist admissible states such that
\begin{equation}
F(S) \neq F(C_j(S)).
\end{equation}
\end{proposition}
...
\newblock Local manuscript draft, 2026.
